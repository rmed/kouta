...
namespace kouta::tests::db
{
    ...
    TEST(DbTest, SqliteOperation)
    {
            ...
            Result<int> result = tester.adapter().run_query(
                [](soci::session& s) -> Result<int>
                {
                    for (int i{}; i < 10; ++i)
                    {
                        ...
                        s << "INSERT INTO table1(id, name) values(:id, :name)", soci::use(i), soci::use(name);
                    }
        ...
        // Will result in an UnknownError because SQLite-specific errors are not handled.
        {
            ...
            ASSERT_EQ(result.code, ResultCode::SystemError);
            ASSERT_EQ(result.error_detail, 0);
        }
    }

    /// @brief Test the session reuse mechanism when connected to SQLite
    ///
    /// @details
    /// The test succeeds if all queries are executed correctly.
    TEST(DbTest, SqliteSessionReuse)
    {
        // Use only one session to check there is no starvation
        Tester tester{1};
        tester.connect(":memory:");

        {
            Result<> result = tester.adapter().run_query(
                [&tester](soci::session& s) -> Result<>
                {
                    return tester.adapter().run_query(
                        [&tester](soci::session& s) -> Result<>
                        {
                            return tester.adapter().run_query(
                                [&tester](soci::session& s) -> Result<>
        ...
        // Should have invalidated the lease, try again
        {
            Result<> result = tester.adapter().run_query(
                [&tester](soci::session& s) -> Result<>
                {
        ...
        // Clean leases and retry
        tester.client().clean_leases();

        {
            ...
            ASSERT_EQ(result.code, ResultCode::ConnectionError);
        }
    }
}  // namespace kouta::tests::db
