...
namespace kouta::db
{
    ...
    public:
        ...
        /// @brief Set the internal pointer to the client to use in order to connect to the database.
        ///
        /// @warning
        /// It is dangerous to modify the client while the adapter is performing work.
        ///
        /// @param[in] client           Pointer to the client. Set to `nullptr` to disable the adapter.
    ...
    protected:
        /// @brief Obtain a pointer to the client.
        ///
        /// @note The client must have been initialized beforehand via the @ref set_client() method.
        ///
        /// @returns Pointer to the client if initialized, `nullptr` otherwise.
        Client* client()
        {
            return m_client;
        }
        ...
        ///
        /// This method is re-entrant thanks to the session leasing mechanism implemented by the client, which will
        /// always return the same session when there is an existing lease for the current thread. However, there may be
        /// side effects regarding the state of the session if it is used to perform several queries or transactions in
        /// parallel. It is the responsibility of the developer to make sure that never happens.
        ///
        /// @note If the client has not been initialized, no query will be executed.
        ///
        ...
        std::invoke_result_t<TFunctor, soci::session&> session_run(TFunctor&& functor)
        {
            if (m_client == nullptr)
            {
                log_error("Database client was not initialized");
                return {ResultCode::ConnectionError};
            }

            try
            {
                Client::SessionLease session{m_client->lease_session()};

                if (!session)
                {
                    // Could not lease a session
                    log_error("Could not lease session from client");
                    return {ResultCode::ConnectionError};
                }

                return functor(*session);
            }
    ...
    private:
        ...
        /// @brief Pointer to the database client used.
        Client* m_client;
