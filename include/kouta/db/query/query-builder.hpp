...
namespace kouta::db::query
{
    ...
    public:
        ...
        ///
        /// @param[in] ordering             Pairs of column name and order for the specified column.
        /// @{
        virtual QueryBuilder& order_by(std::initializer_list<std::pair<std::string_view, Order>> ordering)
        {
            return order_by(std::vector<std::pair<std::string_view, Order>>{ordering});
        }

        virtual QueryBuilder& order_by(const std::vector<std::pair<std::string_view, Order>>& ordering)
        {
            if (ordering.empty())
            {
                return *this;
            ...
            std::size_t last_index{ordering.size() - 1};

            for (std::size_t i{}; i < ordering.size(); ++i)
            {
                m_query << ordering[i].first << " " << ((ordering[i].second == Order::Descending) ? "DESC" : "ASC");
